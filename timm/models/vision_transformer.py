...
from timm.utils.binarization_bibert import *
...
class Attention1w1a(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = QuantizeLinear(dim, dim * 3, bias=qkv_bias)
        ...
        self.moveq = LearnableBias(num_heads, seq_len)
        self.movek = LearnableBias(num_heads, seq_len)
        self.movev = LearnableBias(num_heads, seq_len)
        
        self.movefc = LearnableBias2(seq_len)
        self.dense_knowledge = dense_knowledge
        
    def fun(self, x, moveq, movek, movev):
        B, N, C = x.shape
        H, W = 14, 14
        ...
        attn_d = attn.detach()
        tmp1 = F.softmax(attn, dim=-1)
        tmp2 = F.softmax(attn_d, dim=-1)
        
        aa, _ = torch.max(tmp2, dim=-1)
        aa = aa.unsqueeze(-1)
        attn_d = (tmp2 >= (aa*0.25)) * 1.
        
        # back propagation
        attn = attn_d + tmp1 - tmp2
    ...
    def forward(self, x):
        H, W = 14, 14
        B, N, C = x.shape
        res = self.fun(x, self.moveq, self.movek, self.movev)
        x = res
        
        x = self.movefc(x)
        x = x.transpose(1, 2).reshape(B, N, C)
        self.dense_knowledge[0].append(x)  # attention score
...
class Block1w1a(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention1w1a(
            ...
            seq_len=seq_len,
            dense_knowledge = dense_knowledge,
        )
        self.ls1 = LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        self.drop_path1 = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        self.norm2 = norm_layer(seq_len)
        self.mlp = Mlp1w1a(
            in_features=dim,
        ...
        # self.learnablescaling = nn.Parameter(torch.zeros(1,1,dim), requires_grad=True)
    ...
    def forward(self, x):
        ...
        self.dense_knowledge[1].append(x) 
        
        # phase 2
        # prev = x
        # x1 = self.mlp(self.norm2(x))
        # x2 = self.mlp2(self.norm3(x.transpose(1, 2))).transpose(1, 2)
        # x = x1 + self.learnablescaling * x2
        # x = prev + self.drop_path2(self.ls2(x))
...
class VisionTransformer1w1a(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        norm_layer = norm_layer or partial(nn.BatchNorm1d, eps=1e-6)
        ...
        # embed_len = 196  # modify
        self.pos_embed = nn.Parameter(torch.randn(1, embed_len, embed_dim) * .02) # modify
        ...
        self.norm_pre = norm_layer(embed_len) if pre_norm else nn.Identity()

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        
        self.dense_knowledge = [[] for _ in range(4)]
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                mlp_layer=mlp_layer,
                seq_len = embed_len,
                dense_knowledge = self.dense_knowledge,
            )
            for i in range(depth)])
        
        self.norm = norm_layer(embed_len) if not use_fc_norm else nn.Identity()
    ...
    def forward(self, x):
        for i in range(4):
            self.dense_knowledge[i].clear()
        x = self.forward_features(x)
        x = self.forward_head(x)
        return x, self.dense_knowledge[0], self.dense_knowledge[1], self.dense_knowledge[2], self.dense_knowledge[3]
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # self.dense_knowledge[0].append(torch.matmul(q, q.transpose(-1, -2)))
        # self.dense_knowledge[0].append(torch.matmul(k, k.transpose(-1, -2)))
        # self.dense_knowledge[0].append(torch.matmul(v, v.transpose(-1, -2)))
        ...
        attn = self.attn_drop(attn)
        # self.dense_knowledge[0].append(torch.matmul(attn, attn.transpose(-1, -2)))  # attention score
        x = attn @ v
...
def _create_vision_transformer(variant, pretrained=False, bnn=False, **kwargs):
...
@register_model
def vit_tiny_patch16_224_1w1a(pretrained=False, **kwargs) -> VisionTransformer1w1a:
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, block_fn=Block1w1a)
    model = _create_vision_transformer('vit_tiny_patch16_224_1w1a', bnn=True, pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
@register_model
def vit_small_patch16_224_1w1a(pretrained=False, **kwargs) -> VisionTransformer1w1a:
    """ ViT-Small (ViT-S/16)
    """    
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, block_fn=Block1w1a)
    model = _create_vision_transformer('vit_small_patch16_224_1w1a', bnn=True, pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
@register_model
def vit_base_patch16_224_1w1a(pretrained=False, **kwargs) -> VisionTransformer:
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, block_fn=Block1w1a)
    model = _create_vision_transformer('vit_base_patch16_224_1w1a', bnn=True, pretrained=pretrained, **dict(model_args, **kwargs))
    return model
