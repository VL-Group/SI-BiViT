...
from timm.utils.binarization_bibert import BinaryQuantizer, QuantizeLinear, LearnableBias, Mlp1w1a
...
class WindowAttention1w1a(nn.Module):
    ...
    def __init__(
        ...
        # self.qkv = QuantizeLinear(dim, attn_dim * 3, bias=qkv_bias)
        self.qkv = nn.Linear(dim, attn_dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = QuantizeLinear(attn_dim, dim)
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        # get threshold T
        attn_d = attn.detach()
        aa, _ = torch.max(attn_d, dim=-1)
        aa = aa.unsqueeze(-1)
        attn_d = (attn_d >= (aa*0.25))*1.0
        attn_d = BinaryQuantizer().apply(attn_d)
...
class SwinTransformerBlock1w1a(nn.Module):
...
@register_model
def swin_tiny_patch4_window7_224_1w1a(pretrained=False, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_args = dict(patch_size=4, window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), block_fn=SwinTransformerBlock1w1a)
    return _create_swin_transformer(
        'swin_tiny_patch4_window7_224_1w1a', pretrained=pretrained, **dict(model_args, **kwargs))
